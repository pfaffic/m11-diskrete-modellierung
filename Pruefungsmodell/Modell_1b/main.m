...
axis([0 t+10 0 1.3])
  if (plotteInMehrereGrafiken)
    title(['Lockdown am Tag: ' num2str(lockdown) "  Gebiet: " num2str(g)])
  else
     title(['Lockdown am Tag: ' num2str(lockdown) "  Alle Gebiete"])
  endif
  ...
  if(plotsAlsBilderSpeichern && !plotteInMehrereGrafiken)
    filename=["GLP_Modell_1b_Matrix_" num2str(matrix) "_Alle_Gebiete_Lockdown_" num2str(lockdown) ".jpg"];
    saveas((1), filename);
  elseif (plotsAlsBilderSpeichern && plotteInMehrereGrafiken)
    filename=["GLP_Modell_1b_Matrix_" num2str(matrix) "_Gebiet_" num2str(g) "_Lockdown_" num2str(lockdown) ".jpg"];
    saveas((g), filename);
  endif
endfor
